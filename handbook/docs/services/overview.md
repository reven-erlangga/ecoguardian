...
Ecoguard has **9 backend services** connected via gRPC and RabbitMQ.
...
| Service | Language | gRPC Port | HTTP Port | Database | Proto |
|---------|----------|-----------|-----------|----------|-------|
| **Gateway** | Node.js | - | 4000 | - | - |
| **User Auth** | Python | 50051 | - | PostgreSQL | `user` |
| **Twitter** | Rust | 50052 | 8000 | MongoDB | `twitter` |
| **Classification** | Python | 50053 | 8083 | ONNX | `classification` |
...
sequenceDiagram
    ...
    Note over G,S: port 50051-50058
    S-->>G: gRPC Response
```
...
```mermaid
graph LR
    TW[Twitter] -->|tweet.ingested| RMQ[(RabbitMQ)]
    CL[Classification] -->|classification.completed| RMQ
    IS[Issue] -->|issue.created| RMQ
