...
def _doc_to_issue(doc: dict) -> issue_pb2.Issue:
    ...
    return issue_pb2.Issue(
        ...
        resolution=issue_pb2.Resolution(
            ...
            image_hash=res.get("image_hash", ""), resolved_at=int(res.get("resolved_at", 0)),
...
class IssueServicer(service_pb2_grpc.IssueServiceServicer):
    ...
    def ListIssues(self, request, context):
        ...
        items, total = self.repo.list_issues(
            status=request.status, type_filter=request.type,
...
if __name__ == "__main__":
    cfg = Config()
    repo = IssueRepository(mongo_uri=cfg.MONGODB_URI)
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    service_pb2_grpc.add_IssueServiceServicer_to_server(IssueServicer(repo), server)
