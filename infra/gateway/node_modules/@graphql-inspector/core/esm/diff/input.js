...
export function changesInInputObject(oldInput, newInput, addChange) {
    ...
    compareDirectiveLists(oldInput?.astNode?.directives || [], newInput.astNode?.directives || [], {
        onAdded(directive) {
            addChange(directiveUsageAdded(Kind.INPUT_OBJECT_TYPE_DEFINITION, directive, newInput, oldInput === null));
...
function changesInInputField(input, oldField, newField, addChange) {
    ...
    if (newField.astNode?.directives) {
        compareDirectiveLists(oldField?.astNode?.directives || [], newField.astNode.directives || [], {
            ...
            onMutual(directive) {
                directiveUsageChanged(directive.oldVersion, directive.newVersion, addChange, input, newField);
            },
            onRemoved(directive) {
