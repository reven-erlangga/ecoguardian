...
__export(execution_exports, {
  ...
  buildCompilationContext: () => buildCompilationContext,
...
function compileType(context, parentType, type, fieldNodes, originPaths, destinationPaths, previousPath) {
  ...
  const isNonNull = (0, import_graphql.isNonNullType)(type);
  const innerType = isNonNull ? type.ofType : type;
  if ((0, import_graphql.isLeafType)(innerType) && !(context.options.disableLeafSerialization && (innerType instanceof import_graphql.GraphQLEnumType || (0, import_graphql.isSpecifiedScalarType)(innerType)))) {
    ...
    const locs = JSON.stringify((0, import_ast.computeLocations)(fieldNodes));
    const path = serializeResponsePathAsArray(previousPath);
    const capStack = context.options.disablingCapturingStackErrors ? "true" : "false";
    const serializerRef = `${GLOBAL_EXECUTION_CONTEXT}.serializers.${serializerName}`;
    const extraIndexes = parentIndexes ? `, ${parentIndexes}` : "";
    if (isNonNull) {
      const nullMsg = `"Cannot return null for non-nullable field ${parentType.name}.${getFieldNodesName(fieldNodes)}."`;
...
function compileObjectType(context, type, fieldNodes, originPaths, destinationPaths, responsePath, errorDestination, fieldMap, alwaysDefer) {
  ...
  context.trivialTypeNameVar = void 0;
  ...
  for (const name of Object.keys(fieldMap)) {
    ...
    const alwaysIncluded = fieldCondition.split(" || ").every((p) => p === "true");
    if (!alwaysIncluded) {
      body(`(${fieldCondition})`);
    }
...
function compileAbstractType(context, parentType, type, fieldNodes, originPaths, previousPath, errorDestination) {
  ...
  const trivialGroups = /* @__PURE__ */ new Map();
...
function buildCompilationContext(schema, document, options, operationName) {
  ...
  return {
    ...
    leafErrHandlerCache: /* @__PURE__ */ new Map(),
    deferred: [],
    depth: -1,
    variableValues: {},
    errors
