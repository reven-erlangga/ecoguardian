...
async function getMesh(options) {
    const rawSources = [];
    const { pubsub = new utils_1.PubSub(), cache, logger = new utils_1.DefaultLogger(''), additionalEnvelopPlugins = [], sources, merger, additionalResolvers = [], additionalTypeDefs = [], transforms = [], fetchFn = fetch_1.fetch, } = options;
    ...
    const plugins = [
        ...
        (0, core_1.useExtendContext)(() => {
            if (!inContextSDK) {
                ...
                inContextSDK = (0, utils_1.getInContextSDK)(schema, rawSources, logger, onDelegateHooks);
            }
            return inContextSDK;
        }),
        {
    ...
    function createExecutor(globalContext) {
        ...
        return function meshExecutor(documentOrSDL, variableValues, contextValue, rootValue, operationName) {
            const globalContextClone = globalContext ? Object.create(globalContext) : undefined;
            const { schema, parse, execute, subscribe, contextFactory } = getEnveloped(globalContextClone);
