...
async function processConfig(config, options) {
    ...
    const { cache, code: cacheCode } = await (0, utils_js_1.resolveCache)(config.cache, importFn, rootStore, dir, pubsub, logger, additionalPackagePrefixes);
    codes.add(cacheCode);
    const { fetchFn, code: fetchFnCode } = await (0, utils_js_1.resolveCustomFetch)({
    ...
    const [sources, transforms, additionalEnvelopPlugins, additionalTypeDefs, additionalResolvers, documents,] = await Promise.all([
            ...
            const [handler, transforms] = await Promise.all([
                await (0, utils_js_1.getPackage)({
                    ...
                    if (options.generateCode) {
                        const handlerImportName = (0, pascal_case_1.pascalCase)(handlerVariableName);
                        codes.add(`const ${handlerImportName} = await import(${JSON.stringify(moduleName)}).then(handleImport);\n` +
              ...
              importFn,
                ...
                Promise.all((source.transforms || []).map(async (t, transformIndex) => {
                    ...
                    if (options.generateCode) {
                        const transformImportName = (0, pascal_case_1.pascalCase)(transformsVariableName + '_' + transformIndex);
                        codes.add(`const ${transformImportName} = await import(${JSON.stringify(moduleName)}).then(handleImport);\n` +
        ...
        Promise.all(config.transforms?.map(async (t, transformIndex) => {
            ...
            if (options.generateCode) {
                const transformImportName = (0, pascal_case_1.pascalCase)('Root_Transform_' + transformIndex);
                codes.add(`const ${transformImportName} = await import(${JSON.stringify(moduleName)}).then(handleImport);\n` +
        ...
        Promise.all(config.plugins?.map(async (p, pluginIndex) => {
            ...
            if (ENVELOP_CORE_PLUGINS_MAP[pluginName] != null) {
                ...
                if (options.generateCode) {
                    ...
                    codes.add(`const ${importName} = await import(${JSON.stringify(moduleName)}).then(m => m?.${importProp});`);
            ...
            if (typeof possiblePluginFactory === 'function') {
                ...
                if (options.generateCode) {
                    const importName = (0, camel_case_1.camelCase)('use_' + pluginName);
                    codes.add(`const ${importName} = await import(${JSON.stringify(moduleName)}).then(handleImport);`);
    ...
    if (options.generateCode) {
        codes.add(`const Merger = await import(${JSON.stringify(mergerModuleName)}).then(handleImport);`);
    ...
    if (config.additionalEnvelopPlugins) {
        codes.add(`const importedAdditionalEnvelopPlugins = await import(${JSON.stringify(cross_helpers_1.path.join('..', config.additionalEnvelopPlugins).split('\\').join('/'))}).then(handleImport);`);
    ...
    if (documents?.length) {
        ...
        ${[...documentHashMapCodes].join(',\n')}
            ...
            codes.add(`const usePersistedOperations = await import('@graphql-yoga/plugin-persisted-operations').then(m => m?.usePersistedOperations);`);
