...
export async function processConfig(config, options) {
    ...
    const { cache, code: cacheCode } = await resolveCache(config.cache, importFn, rootStore, dir, pubsub, logger, additionalPackagePrefixes);
    codes.add(cacheCode);
            ...
            const [handler, transforms] = await Promise.all([
              ...
              importFn,
                ...
                Promise.all((source.transforms || []).map(async (t, transformIndex) => {
                    ...
                    if (options.generateCode) {
                        const transformImportName = pascalCase(transformsVariableName + '_' + transformIndex);
    ...
    if (config.additionalEnvelopPlugins) {
        codes.add(`const importedAdditionalEnvelopPlugins = await import(${JSON.stringify(pathModule.join('..', config.additionalEnvelopPlugins).split('\\').join('/'))}).then(handleImport);`);
