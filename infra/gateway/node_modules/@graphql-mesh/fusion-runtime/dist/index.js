  ...
  if (executableUnifiedGraph.getDirective("pubsubOperation")) {
    executableUnifiedGraph = mapSchema(executableUnifiedGraph, {
      [MapperKind.ROOT_FIELD](fieldConfig) {
        const directiveExtensions = getDirectiveExtensions(fieldConfig, executableUnifiedGraph);
        if (directiveExtensions.pubsubOperation?.length) {
          for (const pubsubOperationArgs of directiveExtensions.pubsubOperation) {
            const { subscribe, resolve } = getResolverForPubSubOperation(pubsubOperationArgs);
...
function wrapExecutorWithHooks({
  ...
  function executorWithHooks(baseExecutionRequest) {
    ...
    const log = transportContext?.log.child({ subgraph: subgraphName }) || new Logger({ attrs: { subgraph: subgraphName } });
