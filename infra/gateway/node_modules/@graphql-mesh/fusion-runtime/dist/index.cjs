...
function getMeshSourceDirectiveName(schemaExtensions) {
  const linkDirectives = schemaExtensions?.["directives"]?.link;
  if (linkDirectives) {
    for (const linkDirective of linkDirectives) {
      if (linkDirective.url === "https://the-guild.dev/graphql/mesh/spec/v1.0") {
        for (const importInfo of linkDirective.import || []) {
          if (importInfo === "@source") {
            ...
            if (typeof alias === "string") {
              return alias.startsWith("@") ? alias.slice(1) : void 0;
            }
            return void 0;
          }
        }
...
function handleFederationSubschema({
  ...
  const meshSourceDirectiveName = getMeshSourceDirectiveName(subgraphExtensions);
  ...
  subschemaConfig.schema = utils.mapSchema(subschemaConfig.schema, {
    [utils.MapperKind.TYPE]: (type) => {
      ...
      if (meshSourceDirectiveName) {
        const sourceDirectives = typeDirectives[meshSourceDirectiveName];
    ...
    [utils.MapperKind.OBJECT_FIELD]: (fieldConfig, fieldName, typeName, schema) => {
      ...
      const sourceDirectives = meshSourceDirectiveName ? fieldDirectives[meshSourceDirectiveName] : void 0;
      const sourceDirective = sourceDirectives?.find(
        (directive) => compareSubgraphNames(directive.subgraph, subgraphName)
      );
      ...
      if (fieldConfig.args) {
        for (const argName in fieldConfig.args) {
          ...
          const argSourceDirectives = meshSourceDirectiveName ? argDirectives[meshSourceDirectiveName] : void 0;
    ...
    [utils.MapperKind.ENUM_VALUE]: (enumValueConfig, typeName, _schema, externalValue) => {
      ...
      const sourceDirectives = meshSourceDirectiveName ? enumDirectives[meshSourceDirectiveName] : void 0;
      const sourceDirective = sourceDirectives?.find(
        (directive) => compareSubgraphNames(directive.subgraph, subgraphName)
      );
      if (sourceDirective != null) {
...
const handleFederationSupergraph = function({
  ...
  log: rootLog = new logger.Logger({ level: false })
}) {
  ...
  const inContextSDK = utils$1.getInContextSDK(
    ...
    subschemas,
    logger.LegacyLogger.from(rootLog),
    onDelegateHooks || []
  );
  ...
  if (executableUnifiedGraph.getDirective("pubsubOperation")) {
    executableUnifiedGraph = utils.mapSchema(executableUnifiedGraph, {
      [utils.MapperKind.ROOT_FIELD](fieldConfig) {
        const directiveExtensions = utils.getDirectiveExtensions(fieldConfig, executableUnifiedGraph);
        if (directiveExtensions.pubsubOperation?.length) {
          for (const pubsubOperationArgs of directiveExtensions.pubsubOperation) {
            const { subscribe, resolve } = utils$1.getResolverForPubSubOperation(pubsubOperationArgs);
    ...
    executableUnifiedGraph = utils.mapSchema(executableUnifiedGraph, {
      [utils.MapperKind.ROOT_FIELD](fieldConfig) {
        ...
        if (directiveExtensions.pubsubPublish?.length) {
          const originalResolve = fieldConfig.resolve || delegate.defaultMergedResolver;
  ...
  function getSubschema(subgraphName) {
  ...
  const overrideLabels = executableUnifiedGraph.extensions?.["overrideLabels"];
  return {
...
function getTransportExecutor({
  ...
  return promiseHelpers.handleMaybePromise(
    ...
    (transport) => {
      ...
      const log = transportContext?.log || // if the logger is not provided by the context, create a new silent one just for consistency in the hooks
      new logger.Logger({ level: false });
      const logger$1 = transportContext?.logger || logger.LegacyLogger.from(log);
      return getSubgraphExecutor({
        subgraphName,
        subgraph,
        transportEntry,
...
function wrapExecutorWithHooks({
  ...
  function executorWithHooks(baseExecutionRequest) {
    ...
    const log = transportContext?.log.child({ subgraph: subgraphName }) || new logger.Logger({ attrs: { subgraph: subgraphName } });
  ...
  return function instrumentedExecutor(executionRequest) {
    const subgraphInstrument = instrumentation$1()?.subgraphExecute;
    return instrumentation.getInstrumented({ executionRequest, subgraphName }).asyncFn(
      subgraphInstrument,
      executorWithHooks
    )(executionRequest);
  };
...
function wrapMergedTypeResolver(originalResolver, typeName, onDelegationStageExecuteHooks, log) {
...
class UnifiedGraphManager {
  ...
  executor;
  instrumentation;
  overrideLabelsByContext = /* @__PURE__ */ new WeakMap();
  overrideLabels;
  ensureUnifiedGraph() {
    ...
    if (loadedUnifiedGraph != null) {
      ...
      return promiseHelpers.handleMaybePromise(
        () => this.handleUnifiedGraph({
          ...
          handleProgressiveOverride: this.opts.handleProgressiveOverride ? (label, context) => {
      ...
      () => {
        ...
        if (handleProgressiveOverride && this.overrideLabels) {
          ...
          for (const label of this.overrideLabels) {
            ...
            const handleResult = (shouldEnable) => {
              if (shouldEnable) {
            ...
            if (promiseHelpers.isPromise(result$)) {
...
function serializeLoadedUnifiedGraph(loadedUnifiedGraph) {
  if (typeof loadedUnifiedGraph === "string") {
    return loadedUnifiedGraph;
  }
  if (graphql.isSchema(loadedUnifiedGraph)) {
    return utils.printSchemaWithDirectives(loadedUnifiedGraph);
  }
  return transportCommon.defaultPrintFn(loadedUnifiedGraph);
}
