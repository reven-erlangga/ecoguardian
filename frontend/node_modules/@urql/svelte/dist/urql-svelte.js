...
function queryStore(args) {
  ...
  var operation$ = store.writable(operation);
  var initialState = {
    ...initialResult,
    operation
  };
  ...
  var result$ = store.writable(initialState, () => {
    return wonka.subscribe(result => {
      ...
      return wonka.switchMap(operation => {
  ...
  var reexecute = context => {
    var newContext = {
      ...context,
      ...args.context
    };
    var operation = args.client.createRequestOperation('query', request, newContext);
    isPaused$.set(false);
...
function subscriptionStore(args, handler) {
  ...
  var result$ = store.writable(initialState, () => {
    return wonka.subscribe(result => {
      ...
      var data = partial.data != null ? typeof handler === 'function' ? handler(result.data, partial.data) : partial.data : result.data;
      return {
        ...result,
        ...partial,
        data
