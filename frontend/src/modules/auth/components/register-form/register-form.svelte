...
<form onsubmit={submit} class="space-y-4">
  <div>
    ...
    <input id="reg-email" type="email" required bind:value={email}
  ...
  <div>
    ...
    <input id="reg-username" type="text" required bind:value={username}
      class="mt-1 block w-full neo-border bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
  </div>
  <div>
    <label for="reg-password" class="block text-sm font-medium text-foreground">Password</label>
    <input id="reg-password" type="password" required bind:value={password}
      class="mt-1 block w-full neo-border bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
  </div>
  {#if error}
  ...
  <button type="submit" disabled={loading}
    class="neo-border neo-shadow w-full bg-green-600 text-white px-4 py-3 font-bold text-base hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_rgba(0,0,0,1)] disabled:opacity-50 transition-all duration-100">
