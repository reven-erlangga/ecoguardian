...
<div class="space-y-6">
  <div class="neo-border bg-white neo-shadow p-6">
    <h2 class="text-2xl font-bold mb-4">Simulasi Flow Ecoguard</h2>
    <p class="mb-4 text-gray-600">Test end-to-end dari register sampai blockchain</p>
    <button onclick={runSimulation} disabled={running}
      class="neo-border neo-shadow bg-blue-600 text-white px-6 py-3 font-bold text-lg hover:translate-x-[2px] hover:translate-y-[2px] disabled:opacity-50 transition-all">
      {running ? 'Menjalankan...' : '▶ Jalankan Simulasi'}
    </button>
  </div>

  {#if logs.length > 0}
    <div class="neo-border bg-black text-green-400 p-4 font-mono text-sm space-y-1 max-h-96 overflow-y-auto">
      {#each logs as log}
        <div>{log}</div>
      {/each}
    </div>
  ...
  {#if result}
    <div class="neo-border bg-white neo-shadow p-4">
      <p class="font-bold text-lg">{result.success ? '✅ Berhasil' : '❌ Gagal'}</p>
    </div>
  {/if}
</div>
