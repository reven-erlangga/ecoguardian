export const LIST_ISSUES = `query($input: issue__ListIssuesRequest_Input) {
  issue_IssueService_ListIssues(input: $input) {
    issues { id tweet_id type confidence status location { lat lon address } paraphrased_text resolution { admin_id notes image_hash resolved_at } created_at }
    pagination { total }
  }
}`;
...
export const GET_ISSUE = `query($input: issue__GetIssueRequest_Input) {
  issue_IssueService_GetIssue(input: $input) {
    issue { id tweet_id type confidence status location { lat lon address } paraphrased_text resolution { admin_id notes image_hash resolved_at } created_at resolved_at image_hash }
  }
}`;
